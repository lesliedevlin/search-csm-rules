...
# leslie add
    def doGetAnnouncements(self):
        url = "%s:%d/%s/system_announcements?status=active" % (self.base_url, self.port, self.api_ver)
        (data, authError) = self.doGetRequest(url, self.authToken)
        if (data):
            return (json.loads(data), authError)
        else:
    ...
    def doGetIssues(self):
        url = "%s:%d/v2/issues" % (self.base_url, self.port)
        (data, authError) = self.doGetRequest(url, self.authToken)
        if (data):
            return (json.loads(data), authError)
        else:
    ...
    def doGetIssuesCSM(self):
        url = "%s:%d/v2/issues?issue_type=csm" % (self.base_url, self.port)
        (data, authError) = self.doGetRequest(url, self.authToken)
        if (data):
            return (json.loads(data), authError)
        else:
    ...
    def doGetPolicies(self):
        url = "%s:%d/v2/policies" % (self.base_url, self.port)
        (data, authError) = self.doGetRequest(url, self.authToken)
        if (data):
            return (json.loads(data), authError)
        else:
    ...
    def doGetCSMPolicyDetails(self, policyID):
#        url = "%s:%d/%s/policies/%s" % (self.base_url, self.port, self.api_ver, policyID)
        url = "%s:%d/v2/policies/%s" % (self.base_url, self.port, policyID)
        (data, authError) = self.doGetRequest(url, self.authToken)
        if (data):
            return (json.loads(data), authError)
        else:
